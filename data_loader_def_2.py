...
def convert_examples_to_features(
    ...
    processor = GoEmotionsProcessorDef(args)
    ...
    for i, example in enumerate(examples[:10]):
        ...
        logger.info("tokens def: {}".format(" ".join([str(x) for x in tokenizer.tokenize(example.text_b)])))
...
class GoEmotionsProcessorDef(object):
    ...
    def get_labels(self):
        labels = []
        with open(os.path.join(self.args.data_dir, self.args.label_def_file), "r", encoding="utf-8") as f:
            for line in f:
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = items[0]
            text_b = items[1]
            label = [int(items[2])]#list(map(int, items[1].split(",")))
            if label[0]==0:
               # logger.info(line)
    ...
    def get_examples(self, mode):
        ...
        file_to_read = None
        if mode == 'train_def':
            file_to_read = self.args.train_def_file
        elif mode == 'dev_def':
            file_to_read = self.args.dev_def_file
        elif mode == 'test_def':
            file_to_read = self.args.test_def_file
...
def load_and_cache_examples_def(args, tokenizer, mode):
    processor = GoEmotionsProcessorDef(args)
    ...
    if os.path.exists(cached_features_file):
        ...
        if mode == "train_def":
            examples = processor.get_examples("train_def")
        elif mode == "dev_def":
            examples = processor.get_examples("dev_def")
        elif mode == "test_def":
            examples = processor.get_examples("test_def")
        else:
